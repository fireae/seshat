...
// frame methods
int frame::get_fr_dim() { return 7; }

// sentenceF methods
sentenceF::sentenceF() : n_frames(0), frames(NULL){};

sentenceF::~sentenceF() { delete[] frames; }
...
vector<PointR> sentenceF::normalizaAspect(vector<Point> &puntos) {
  ...
  for (int i = 0; i < puntos.size(); i++) {
    ...
    PointR p(TAM * ((puntos[i].x - xmin) / (ymax - ymin)),
             TAM * (puntos[i].y - ymin) / (ymax - ymin));
...
void sentenceF::calculate_derivatives(vector<PointR> &points, bool norm) {
  ...
  for (int i = 0; i < points.size(); i++) {
    ...
    for (int c = 1; c <= tamW; c++) {
      ...
      if (norm) {
        double module =
            sqrt(frames[i].dx * frames[i].dx + frames[i].dy * frames[i].dy);
        if (module > 0) {
          frames[i].dx /= module;
          frames[i].dy /= module;
        }
...
void sentenceF::calculate_kurvature() {
  for (int i = 0; i < n_frames; i++) {
    double norma =
        sqrt(frames[i].dx * frames[i].dx + frames[i].dy * frames[i].dy);
    if (norma == 0) norma = 1;
    frames[i].k = (frames[i].dx * frames[i].ay - frames[i].ax * frames[i].dy) /
                  (norma * norma * norma);
  }
}
