...
Hypothesis *TableCYK::getMLH() { return Target; }

CellCYK *TableCYK::get(int n) { return T[n - 1]; }

int TableCYK::size(int n) { return TS[n - 1].size(); }

void TableCYK::updateTarget(coo *K, Hypothesis *H) {
  int pcomps = 0;

  for (int i = 0; i < H->parent->nc; i++)
    if (H->parent->ccc[i]) pcomps++;
...
void TableCYK::add(int n, CellCYK *celda, int noterm_id, bool *esinit) {
  ...
  if (it == TS[n - 1].end()) {
    ...
    if (noterm_id >= 0) {
      if (esinit[noterm_id]) updateTarget(&key, celda->noterm[noterm_id]);
    } else {
    ...
    if (!celda->ccEqual(r)) {
      ...
      // If the new cell contains the most likely hypothesis, replace the
      // hypotheses
      if (maxpr_c > maxpr_r) {
        // Copy the new set of strokes
        for (int i = 0; i < celda->nc; i++) r->ccc[i] = celda->ccc[i];
        ...
        for (int i = 0; i < celda->nnt; i++)
          if (celda->noterm[i]) {
            if (r->noterm[i]) {
              ...
              if (esinit[i]) updateTarget(&key, r->noterm[i]);
            } else {
              r->noterm[i] = celda->noterm[i];
              r->noterm[i]->parent = r;

              // Set to NULL such that the "delete celda" doesn't delete the
              // hypothesis
              celda->noterm[i] = NULL;
