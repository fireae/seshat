...
SymRec::SymRec(char *config) {
  ...
  if (RNNalpha <= 0.0 || RNNalpha >= 1.0) {
    fprintf(stderr,
            "Error: loading config file '%s': must be 0 < RNNalpha < 1\n",
            config);
    exit(-1);
  }
  if (RNNon[0] == 0) {
  ...
  if (conf_on.get<bool>("loadWeights", false)) deh_on.load(conf_on, cout);

  // Offline info
  ConfigFile conf_off(RNNoff);

  // Check if the targetLabels are the same for both online and offline
  // RNN-BLSTM
  vector<string> aux = conf_off.get_list<string>("targetLabels");
  if (aux.size() != header_on.targetLabels.size()) {
    fprintf(stderr,
            "Error: Target labels of online and offline symbol classifiers do "
            "not match\n");
    exit(-1);
  }

  for (vector<string>::iterator it1 = aux.begin(),
                                it2 = header_on.targetLabels.begin();
       it1 != aux.end() && it2 != header_on.targetLabels.end(); it1++, it2++) {
    if ((*it1).compare(*it2)) {
      fprintf(stderr,
              "Error: Target labels of online and offline symbol classifiers "
              "do not match\n");
      exit(-1);
    }
  }
  ...
  if (conf_off.get<bool>("loadWeights", false)) deh_off.load(conf_off, cout);
}
...
char *SymRec::strClase(int c) { return (char *)(key2cl[c]).c_str(); }
...
bool SymRec::checkClase(char *str) {
  if (cl2key.find(str) == cl2key.end()) return false;
  return true;
}

int SymRec::getNClases() { return C; }

// Returns the type of symbol of class k
int SymRec::symType(int k) { return type[k]; }

/************
 * Classify *
 ************/

int SymRec::clasificar(Sample *M, int ncomp, const int NB, int *vclase,
...
int SymRec::clasificar(Sample *M, list<int> *LT, const int NB, int *vclase,
                       float *vpr, int *as, int *ds) {
  SegmentHyp aux;
...
int SymRec::classify(Sample *M, SegmentHyp *SegHyp, const int NB, int *vclase,
                     float *vpr, int *as, int *ds) {
  int regy = INT_MAX, regt = -INT_MAX, N = 0;

  // First compute the vertical centroid (cen) and the ascendant/descendant
  // centroids (as/ds)
  SegHyp->cen = 0;
  for (list<int>::iterator it = SegHyp->stks.begin(); it != SegHyp->stks.end();
       it++) {
    for (int j = 0; j < M->getStroke(*it)->getNpuntos(); j++) {
      Punto *p = M->getStroke(*it)->get(j);

      if (M->getStroke(*it)->ry < regy) regy = M->getStroke(*it)->ry;
      if (M->getStroke(*it)->rt > regt) regt = M->getStroke(*it)->rt;
  ...
  *ds = (regy + SegHyp->cen) / 2;
  ...
  for (int i = 0; i < NB; i++) {
    ...
    for (int j = 0; j < NB && !found; j++)
      if (clasoff[i].second == clason[j].second) found = true;
...
void SymRec::BLSTMclassification(Mdrnn *net, DataSequence *seq,
  ...
  for (int i = 0; i < NCLA; i++)
    prob_class[i].second =
        cl2key[header_on.targetLabels[i]];  // targetLabels on = targetLabels
                                            // off

  for (int i = 0; i < NCLA; i++) prob_class[i].first = 0.0;
  ...
  for (int ncla = 0; ncla < NCLA; ncla++) prob_class[ncla].first /= NVEC;
  ...
  for (int i = 0; i < NB; i++) claspr[i] = prob_class[i];
