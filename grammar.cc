...
Grammar::Grammar(char *path, SymRec *sr) {
  ...
  for (int i = 0; i < (int)noTerminales.size(); i++) esInit[i] = false;
...
void Grammar::addTerminal(float pr, char *S, char *T, char *tex) {
  ...
  bool create = true;
  for (list<ProductionT *>::iterator it = prodTerms.begin();
       it != prodTerms.end(); it++)
...
void Grammar::addRuleH(float pr, char *S, char *A, char *B, char *out,
  ...
  ProductionB *pd = new ProductionH(noTerminales[S], noTerminales[A],
                                    noTerminales[B], pr, out);
...
void Grammar::addRuleV(float pr, char *S, char *A, char *B, char *out,
                       char *merge) {
  if (noTerminales.find(S) == noTerminales.end())
    error("Rule: Non-terminal '%s' not defined.", S);
  if (noTerminales.find(A) == noTerminales.end())
  ...
  ProductionB *pd = new ProductionV(noTerminales[S], noTerminales[A],
                                    noTerminales[B], pr, out);
...
void Grammar::addRuleVe(float pr, char *S, char *A, char *B, char *out,
                        char *merge) {
  if (noTerminales.find(S) == noTerminales.end())
    error("Rule: Non-terminal '%s' not defined.", S);
  if (noTerminales.find(A) == noTerminales.end())
  ...
  ProductionB *pd = new ProductionVe(noTerminales[S], noTerminales[A],
                                     noTerminales[B], pr, out);
...
void Grammar::addRuleSSE(float pr, char *S, char *A, char *B, char *out,
  ...
  ProductionB *pd = new ProductionSSE(noTerminales[S], noTerminales[A],
                                      noTerminales[B], pr, out);
  pd->setMerges(merge[0]);
...
void Grammar::addRuleSup(float pr, char *S, char *A, char *B, char *out,
  ...
  ProductionB *pd = new ProductionSup(noTerminales[S], noTerminales[A],
                                      noTerminales[B], pr, out);
  pd->setMerges(merge[0]);
...
void Grammar::addRuleSub(float pr, char *S, char *A, char *B, char *out,
  ...
  ProductionB *pd = new ProductionSub(noTerminales[S], noTerminales[A],
                                      noTerminales[B], pr, out);
...
void Grammar::addRuleIns(float pr, char *S, char *A, char *B, char *out,
  ...
  ProductionB *pd = new ProductionIns(noTerminales[S], noTerminales[A],
                                      noTerminales[B], pr, out);
...
void Grammar::addRuleMrt(float pr, char *S, char *A, char *B, char *out,
                         char *merge) {
  if (noTerminales.find(S) == noTerminales.end())
    error("Rule: Non-terminal '%s' not defined.", S);
  if (noTerminales.find(A) == noTerminales.end())
  ...
  ProductionB *pd = new ProductionMrt(noTerminales[S], noTerminales[A],
                                      noTerminales[B], pr, out);
...
Grammar::~Grammar() {
  for (list<ProductionB *>::iterator it = prodsH.begin(); it != prodsH.end();
       it++)
    delete *it;

  for (list<ProductionB *>::iterator it = prodsSub.begin();
       it != prodsSub.end(); it++)
    delete *it;

  for (list<ProductionB *>::iterator it = prodsSup.begin();
       it != prodsSup.end(); it++)
    delete *it;

  for (list<ProductionB *>::iterator it = prodsV.begin(); it != prodsV.end();
       it++)
    delete *it;

  for (list<ProductionB *>::iterator it = prodsVe.begin(); it != prodsVe.end();
       it++)
    delete *it;
  ...
  for (list<ProductionB *>::iterator it = prodsIns.begin();
       it != prodsIns.end(); it++)
    delete *it;

  for (list<ProductionB *>::iterator it = prodsMrt.begin();
       it != prodsMrt.end(); it++)
    delete *it;
...
const char *Grammar::key2str(int k) {
  for (map<string, int>::iterator it = noTerminales.begin();
       it != noTerminales.end(); it++) {
    if (it->second == k) return it->first.c_str();
  }
  return "NULL";
}
