...
LogSpace::LogSpace(CellCYK *c, int nr, int dx, int dy) {
  ...
  data = new CellCYK *[N];
  int i = 0;
  for (CellCYK *r = c; r; r = r->sig) data[i++] = r;

  // Sort the regions
  quicksort(data, 0, N - 1);
}

LogSpace::~LogSpace() { delete[] data; }
...
void LogSpace::bsearchStv(int sx, int sy, int ss, int st, list<CellCYK *> *set,
                          bool U_V, CellCYK *cd) {
  // Binary search of "sx"
  int i, j;
  for (i = 0, j = N; i < j;) {
  ...
  if (U_V) {  // Direction 'Up' (U)
    while (i < N && data[i]->x <= ss) {
      if (data[i]->t <= st && data[i]->t >= sy && data[i]->s <= ss) {
        if (data[i]->t < cd->y) sy = max(max(data[i]->y, data[i]->t - RY), sy);
        set->push_back(data[i]);
      }
      i++;
    }
    ...
    while (i < N && data[i]->x <= ss) {
      if (data[i]->y <= st && data[i]->y >= sy && data[i]->s <= ss) {
        if (data[i]->y > cd->t) st = min(min(data[i]->t, data[i]->y + RY), st);
        set->push_back(data[i]);
      }
      i++;
    }
...
void LogSpace::bsearchHBP(int sx, int sy, int ss, int st, list<CellCYK *> *set,
                          CellCYK *cd) {
  // Binary search of "sx"
  int i, j;
  for (i = 0, j = N; i < j;) {
  ...
  while (i < N && data[i]->x <= ss) {
    if (data[i]->y <= st && data[i]->t >= sy) {
      if (data[i]->x > cd->s) ss = min(min(data[i]->s, data[i]->x + RX), ss);
      set->push_back(data[i]);
    }
    i++;
  }
