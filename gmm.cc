...
GMM::GMM(char *model) { loadModel(model); }
...
void GMM::loadModel(char *str) {
  ...
  prior = new float[C];
  for (int i = 0; i < C; i++) fscanf(fd, "%f", &prior[i]);
  ...
  for (int c = 0; c < C; c++) {
    ...
    for (int i = 0; i < G; i++) {
      ...
      for (int j = 0; j < D; j++) fscanf(fd, "%f", &mean[c * G + i][j]);
    }

    // Read mixture weights
    weight[c] = new float[G];
    for (int i = 0; i < G; i++) fscanf(fd, "%f", &weight[c][i]);
  }
...
float GMM::pdf(int c, float *v) {
  ...
  for (int i = 0; i < G; i++) {
    float exponent = 0.0;
    for (int j = 0; j < D; j++)
      exponent += (v[j] - mean[c * G + i][j]) * invcov[c * G + i][j] *
                  (v[j] - mean[c * G + i][j]);

    exponent *= -0.5;

    pr += weight[c][i] * pow(2 * PI, -D / 2.0) * pow(det[c * G + i], -0.5) *
          exp(exponent);
  }
