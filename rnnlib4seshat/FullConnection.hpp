...
struct FullConnection : public Connection {
  ...
  // functions
  FullConnection(Layer* f, Layer* t, WeightContainer* weight,
                 const vector<int>& d = empty_list_of<int>(),
                 FullConnection* s = 0)
      : Connection(make_name(f, t, d), f, t),
        source(s)
  // paramRange(source ? source->paramRange :
  // wc->new_parameters(this->from->output_size() * this->to->input_size(),
  // this->from->name, this->to->name, name))
  {
    wc = weight;
    if (source) {
      paramRange = source->paramRange;
      wc->link_layers(this->from->name, this->to->name, this->name,
  ...
  const View<real_t> weights() { return wc->get_weights(paramRange); }
  const View<real_t> derivs() { return wc->get_derivs(paramRange); }
