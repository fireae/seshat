...
template <class T>
ostream& operator<<(ostream& out, const Log<T>& l) {
  out << l.log();
  return out;
}
template <class T>
istream& operator>>(istream& in, Log<T>& l) {
  T d;
  in >> d;
  l = Log<T>(d, true);
  return in;
...
template <class T>
const T Log<T>::expMax = numeric_limits<T>::max();
template <class T>
const T Log<T>::expMin = numeric_limits<T>::min();
template <class T>
