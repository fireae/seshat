...
typedef map<std::string, std::string>::iterator MSSI;
typedef map<std::string, std::string>::const_iterator MSSCI;
typedef map<std::string, real_t>::iterator MSDI;
typedef map<std::string, real_t>::const_iterator MSDCI;
typedef map<std::string, pair<int, real_t> >::iterator MSPIDI;
typedef map<std::string, pair<int, real_t> >::const_iterator MSPIDCI;
typedef std::vector<map<std::string, pair<int, real_t> > >::const_iterator
    VMSDCI;
typedef std::vector<map<std::string, pair<int, real_t> > >::iterator VMSDI;
typedef std::vector<map<std::string, pair<int, real_t> > >::reverse_iterator
    VMSDRI;
typedef map<std::string, int>::iterator MSII;
typedef map<std::string, int>::const_iterator MSICI;
...
typedef std::pair<const std::string, real_t> PCSD;
...
typedef const boost::tuple<int, std::string>& TIS;
...
// MISC FUNCTIONS
static bool warn_unless(bool condition, const std::string& str,
                        ostream& out = cout) {
  if (!condition) {
...
static std::string time_stamp(
    const std::string& format = "%Y.%m.%d-%H.%M.%S%F%Q") {
...
template <class T>
static real_t real(const T& t) {
  return lexical_cast<real_t>(t);
}
template <class T>
static int integer(const T& t) {
  return lexical_cast<int>(t);
}
template <class T>
static size_t natural(const T& t) {
...
static std::vector<typename boost::range_value<R1>::type>& select_channels(
  ...
  static std::vector<typename boost::range_value<R1>::type> v;
  v.clear();
  LOOP(int i, channels) {
...
template <class R>
static size_t count_adjacent(const R& r) {
  size_t count = 0;
...
static integer_range<T2> span(T1 t1, T2 t2) {
  return irange<T2>((t1 < t2 ? static_cast<T2>(t1) : t2), t2);
  // return std::make_pair(counting_iterator<T2>(
  //    t1 < t2 ? static_cast<T2>(t1) : t2), counting_iterator<T2>(t2));
}
// template <class T> static pair<counting_iterator<T>, counting_iterator<T> >
// span(const T& t)
template <class T>
static integer_range<T> span(T t) {
  return span(0, t);
  // return std::make_pair(counting_iterator<T>(0), counting_iterator<T>(t));
}
// template <class R>
// static pair<
...
template <class T1, class T2>
static std::vector<T1> iota(const T2& t) {
  std::vector<T1> v;
  LOOP(const T2& val, span(t)) { v += lexical_cast<T1>(val); }
  return v;
}
template <class T1, class T2>
static std::vector<T1> iota(const T2& t1, const T2& t2) {
  std::vector<T1> v;
...
template <class R>
static R& reverse_sort(R& r) {
  reverse(sort(r));
  return r;
}
template <class R>
pair<typename range_value<R>::type, typename range_value<R>::type> minmax(
    const R& r) {
  pair<typename range_const_iterator<R>::type,
       typename range_const_iterator<R>::type>
      p = boost::minmax_element(boost::begin(r), boost::end(r));
...
template <class R, class T>
void vector_assign(const R& r, std::vector<T>& v) {
  v.resize(boost::size(r));
  copy(r, v);
}
template <class R>
static void range_negate_equals(R& r) {
  transform(boost::begin(r), boost::end(r), boost::begin(r),
            negate<typename boost::range_value<R>::type>());
}
template <class R>
static std::vector<typename boost::range_value<R>::type>& range_negate(
    const R& r) {
...
template <class R>
static std::vector<typename boost::range_value<R>::type>& flip(const R& r) {
  static std::vector<typename boost::range_value<R>::type> v;
  v.resize(boost::size(r));
  reverse_copy(r, v);
  return v;
...
template <class R>
static typename range_value<R>::type max(const R& r) {
  return *max_element(boost::begin(r), boost::end(r));
}
template <class C, class Tr, class R>
static void print_range(std::basic_ostream<C, Tr>& out, const R& r,
...
template <typename C>
C inner_product(real_t* a, real_t* aEnd, real_t* b, C c) {
  return inner_product(const_cast<const real_t*>(a),
                       const_cast<const real_t*>(aEnd),
                       const_cast<const real_t*>(b), c);
}
...
template <class R1, class R2>
static typename range_value<R1>::type inner_product(
    const R1& a, const R2& b, typename range_value<R1>::type c = 0) {
  return inner_product(boost::begin(a), boost::end(a), boost::begin(b), c);
}
template <class R>
static typename range_value<R>::type norm(const R& r) {
...
template <class R>
static typename range_value<R>::type product(const R& r) {
  return accumulate(boost::begin(r), boost::end(r),
                    (typename range_value<R>::type)1,
                    multiplies<typename range_value<R>::type>());
}
template <class R>
static typename range_value<R>::type sum(const R& r) {
  return accumulate(boost::begin(r), boost::end(r),
                    (typename range_value<R>::type)0);
}
template <class R>
static typename range_value<R>::type abs_sum(const R& r) {
...
template <class R>
static typename range_value<R>::type log_sum(const R& r) {
  typename range_const_iterator<R>::type e = boost::end(r);
  typename range_value<R>::type v = 0;
  for (typename range_const_iterator<R>::type it = boost::begin(r); it != e;
       ++it) {
...
template <class R>
static typename range_value<R>::type mean(const R& r) {
  return sum(r) / (typename range_value<R>::type)boost::size(r);
}
template <class R>
static typename range_value<R>::type variance(const R& r) {
...
template <class R>
static typename range_value<R>::type std_dev(const R& r) {
...
// PAIR OPERATIONS
template <class T>
static bool in_open_interval(pair<T, T> interval, T val) {
  return ((val > interval.first) && (val < interval.second));
}
template <class T>
static bool in_closed_interval(pair<T, T> interval, T val) {
  return ((val >= interval.first) && (val <= interval.second));
}
template <class T1, class T2>
...
template <class T1, class T2>
static real_t pair_sum(const pair<T1, T2>& p) {
  return (real_t)(p.first + p.second);
}
template <class T1, class T2>
static real_t pair_mean(const pair<T1, T2>& p) {
  return pair_sum(p) / 2.0;
}
template <class T1, class T2>
static size_t difference(const pair<T1, T2>& p) {
...
template <class T1, class T2>
static T2 sum_right(const map<T1, T2>& m) {
  T2 ret = 0;
  for (typename map<T1, T2>::const_iterator it = m.begin(); it != m.end();
       ++it) {
    ret += it->second;
...
// IO OPERATIONS
template <class T>
static void print(const T& t, ostream& out = cout) {
  out << t << endl;
}
template <class T1, class T2>
...
static T read(const std::string& data) {
  ...
  std::stringstream ss;
  ss << boolalpha << data;
  check(ss >> val, "cannot read std::string '" + data +
                       "' into variable with type '" + typeid(T).name() + "'");
  return val;
}
// SET OPERATIONS
template <class T>
static bool disjoint(const set<T>& s1, const set<T>& s2) {
  static set<T> result;
  result.clear();
...
template <class T>
static assign_detail::generic_list<T> empty_list_of() {
