...
void Sample::setSymRec(SymRec *sr) { SR = sr; }
...
void Sample::loadInkML(char *str) {
  ...
  XMLCh *TAG_trace, *TAG_traceFormat, *TAG_traceGroup, *TAG_traceView,
      *TAG_annotation;
  ...
  try {
    ...
    for (XMLSize_t xx = 0; xx < nodeCount; ++xx) {
      ...
      if (currentNode->getNodeType() &&
        ...
        if (xercesc::XMLString::equals(currentElement->getTagName(),
                                       TAG_trace)) {
          ...
          for (XMLSize_t pp = 0; pp < puntos->getLength(); pp++) {
            ...
            if (pt->getNodeType() &&
                pt->getNodeType() == xercesc::DOMNode::TEXT_NODE) {
        ...
        } else if (xercesc::XMLString::equals(currentElement->getTagName(),
                                              TAG_annotation)) {
...
Sample::~Sample() {
  ...
  for (int y = 0; y < Y; y++) delete[] dataoff[y];
  delete[] dataoff;

  for (int i = 0; i < Y; i++) delete[] pix_stk[i];
  delete[] pix_stk;
...
int Sample::get(int x, int y) { return dataoff[y][x]; }

Stroke *Sample::getStroke(int i) { return dataon[i]; }

int Sample::dimX() { return X; }

int Sample::dimY() { return Y; }

int Sample::nStrokes() { return (int)dataon.size(); }
...
void Sample::render_img(char *out) {
  ...
  if (frender) {
    fprintf(frender, "P2\n%d %d\n255\n", X, Y);
    for (int i = 0; i < Y; i++) {
      for (int j = 0; j < X; j++) fprintf(frender, " %3d", dataoff[i][j]);
      fprintf(frender, "\n");
    }
    fclose(frender);
  } else
...
char *Sample::getOutDot() { return outdot; }
...
void Sample::setRegion(CellCYK *c, list<int> *LT) {
  ...
  for (list<int>::iterator it = LT->begin(); it != LT->end(); it++) {
    ...
    if (dataon[*it]->rx < c->x) c->x = dataon[*it]->rx;
    if (dataon[*it]->ry < c->y) c->y = dataon[*it]->ry;
    if (dataon[*it]->rs > c->s) c->s = dataon[*it]->rs;
    if (dataon[*it]->rt > c->t) c->t = dataon[*it]->rt;
  }
}
...
void Sample::setRegion(CellCYK *c, int *v, int size) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (dataon[v[i]]->rx < c->x) c->x = dataon[v[i]]->rx;
    if (dataon[v[i]]->ry < c->y) c->y = dataon[v[i]]->ry;
    if (dataon[v[i]]->rs > c->s) c->s = dataon[v[i]]->rs;
    if (dataon[v[i]]->rt > c->t) c->t = dataon[v[i]]->rt;
  }
}
...
void Sample::printInkML(Grammar *G, Hypothesis *H) {
  ...
  // Save the value of next_id in order to restore it after printing the MathML
  // expression  and generating the IDs for symbols
  int nid_bak = next_id;
  next_id++;  // Skip one ID that will be the traceGroup that starts the symbol
              // part
...
int **Sample::render(int *pW, int *pH) {
  ...
  for (int i = 0; i < H; i++) {
    pix_stk[i] = new int[W];
    for (int j = 0; j < W; j++) pix_stk[i][j] = -1;
  }
...
void Sample::renderStrokesPBM(list<int> *SL, int ***img, int *rows, int *cols) {
  ...
  *img = new int *[*rows];
  for (int i = 0; i < *rows; i++) {
    (*img)[i] = new int[*cols];
    for (int j = 0; j < *cols; j++) (*img)[i][j] = 255;
  }
  ...
  if (SL->size() == 1 && dataon[*(SL->begin())]->getNpuntos() == 1) {
    ...
    for (int i = OFFSET; i < H - OFFSET; i++)
      for (int j = OFFSET; j < W - OFFSET; j++) (*img)[i][j] = 0;
  } else {
    for (list<int>::iterator it = SL->begin(); it != SL->end(); it++)
      for (int i = 0; i < dataon[*it]->getNpuntos(); i++) {
        Punto *p = dataon[*it]->get(i);
  ...
  for (int i = 0; i < *rows; i++) {
    ...
    for (int j = 0; j < *cols; j++) img_smo[i][j] = 0;
  }
  ...
  for (int y = 0; y < *rows; y++)
    for (int x = 0; x < *cols; x++) (*img)[y][x] = img_smo[y][x] < 255 ? 1 : 0;

  for (int y = 0; y < H + 2 * OFFSET; y++) delete[] img_smo[y];
  delete[] img_smo;
}
  ...
  for (int i = 0; i < cd->nc; i++)
    if (cd->ccc[i]) {
      for (int j = 0; j < dataon[i]->getNpuntos(); j++) {
        ...
        if (dataon[i]->ry < regy) regy = dataon[i]->ry;
        if (dataon[i]->rt > regt) regt = dataon[i]->rt;
...
void Sample::compute_strokes_distances(int rx, int ry) {
  ...
  for (int i = 0; i < nStrokes(); i++) stk_dis[i] = new float[nStrokes()];
...
float Sample::stroke_distance(int si, int sj) {
  ...
  if (not_visible(si, sj, min_i, min_j)) dmin = FLT_MAX;
...
// Go through the pixels from pi to pj checking that there is not a pixel that
// belongs  to a stroke that is not si or sj. If so, then sj is not visible from
// si
...
bool Sample::visibility(list<int> *strokes_list) {
  ...
  for (list<int>::iterator it = strokes_list->begin();
       it != strokes_list->end(); it++)
    visited[*it] = false;
  ...
  for (int i = 0; i < A->nc; i++)
    if (A->ccc[i]) {
      for (int j = 0; j < B->nc; j++)
        if (B->ccc[j] && j != i && getDist(i, j) < dmin) dmin = getDist(i, j);
    }
...
void Sample::get_close_strokes(int id, list<int> *L, float dist_th) {
  ...
  for (int i = 0; i < id; i++) added[i] = false;
