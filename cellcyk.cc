...
CellCYK::CellCYK(int n, int ncc) {
  ...
  for (int i = 0; i < nnt; i++) noterm[i] = NULL;

  // Create (empty) strokes covered
  ccc = new bool[nc];
  for (int i = 0; i < nc; i++) ccc[i] = false;
}

CellCYK::~CellCYK() {
  // Delete existing hypotheses
  for (int i = 0; i < nnt; i++)
    if (noterm[i]) delete noterm[i];
...
void CellCYK::ccUnion(CellCYK *A, CellCYK *B) {
  for (int i = 0; i < nc; i++) ccc[i] = (A->ccc[i] || B->ccc[i]) ? true : false;
}

// Check if cell H covers the same strokes that this
bool CellCYK::ccEqual(CellCYK *H) {
  if (talla != H->talla) return false;

  for (int i = 0; i < nc; i++)
    if (ccc[i] != H->ccc[i]) return false;
