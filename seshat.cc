...
void usage(char *str) {
  fprintf(stderr,
          "SESHAT - Handwritten math expression "
          "parser\nhttps://github.com/falvaro/seshat\n");
  fprintf(stderr, "Copyright (C) 2014, Francisco Alvaro\n\n");
  fprintf(stderr,
          "Usage: %s -c config -i input [-o output] [-r render.pgm]\n\n", str);
  fprintf(stderr, "  -c config: set the configuration file\n");
  fprintf(stderr, "  -i input:  set the input math expression file\n");
  fprintf(stderr,
          "  -o output: save recognized expression to 'output' file (InkML "
          "format)\n");
  fprintf(stderr,
          "  -r render: save in 'render' the image representing the input "
          "expression (PGM format)\n");
  fprintf(stderr,
          "  -d graph:  save in 'graph' the description of the recognized tree "
          "(DOT format)\n");
}
...
int main(int argc, char *argv[]) {
  ...
  int option;
  while ((option = getopt(argc, argv, "c:i:o:r:d:")) != -1) switch (option) {
      case 'c':
