...
bool gParser::nextLine(FILE *fd, char *lin) {
  do {
    if (fgets(lin, SIZE, fd) == NULL) return false;
...
void gParser::parse(FILE *fd) {
  ...
  while (nextLine(fd, linea)) {
    ...
    if (!strcmp(tokens[1], "H"))
      ...
      g->addRuleV(atof(tokens[0]), tokens[2], tokens[3], tokens[4], tokens[5],
                  tokens[6]);
    else if (!strcmp(tokens[1], "Ve"))
      g->addRuleVe(atof(tokens[0]), tokens[2], tokens[3], tokens[4], tokens[5],
                   tokens[6]);
      ...
      g->addRuleMrt(atof(tokens[0]), tokens[2], tokens[3], tokens[4], tokens[5],
                    tokens[6]);
    else {
    ...
    // Free memory
    for (int j = 0; j < ntoks; j++) delete[] tokens[j];
    delete[] tokens;
  }
}
