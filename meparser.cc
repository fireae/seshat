...
meParser::meParser(char *conf) {
  ...
  if (qfactor <= 0) fprintf(stderr, "WARNING: SymbolSF = %f\n", qfactor);
  if (ptfactor <= 0) fprintf(stderr, "WARNING: ProductionTSF = %f\n", ptfactor);
  if (pbfactor <= 0) fprintf(stderr, "WARNING: ProductionBSF = %f\n", pbfactor);
  if (rfactor <= 0) fprintf(stderr, "WARNING: RelationSF = %f\n", rfactor);
  if (dfactor < 0) fprintf(stderr, "WARNING: DurationSF = %f\n", dfactor);
  if (gfactor < 0) fprintf(stderr, "WARNING: SegmentationSF = %f\n", gfactor);
...
void meParser::initCYKterms(Sample *M, TableCYK *tcyk, int N, int K) {
  for (int i = 0; i < M->nStrokes(); i++) {
    ...
    bool insertar = false;
    for (list<ProductionT *>::iterator it = G->prodTerms.begin();
         it != G->prodTerms.end(); it++) {
      ProductionT *prod = *it;

      for (int k = 0; k < NB; k++)
        if (pr[k] > 0.0 && prod->getClase(clase[k]) &&
            prod->getPrior(clase[k]) > -FLT_MAX) {
          float prob = log(InsPen) + ptfactor * prod->getPrior(clase[k]) +
                       qfactor * log(pr[k]) +
                       dfactor * log(duration->prob(clase[k], 1));

          if (cd->noterm[prod->getNoTerm()]) {
            if (cd->noterm[prod->getNoTerm()]->pr >
                prob + prod->getPrior(clase[k]))
              continue;
            else
          ...
          // Create new symbol
          cd->noterm[prod->getNoTerm()] =
              new Hypothesis(clase[k], prob, cd, prod->getNoTerm());
...
void meParser::combineStrokes(Sample *M, TableCYK *tcyk, LogSpace **LSP,
                              int N) {
  if (N <= 1) return;
    ...
    for (int size = 2; size <= min(max_strokes, N); size++) {
      ...
      if (size == 2) {
        for (int i = 0; i < stkc1; i++)
          if (M->getDist(stkc1, i) < distance_th) close_list.push_back(i);

      } else
        M->get_close_strokes(stkc1, &close_list, distance_th);

      // If there are not enough strokes to compose a hypothesis of "size",
      // continue
      if ((int)close_list.size() < size - 1) continue;

      int *stkvec = new int[close_list.size()], VS = 0;
      for (list<int>::iterator it = close_list.begin(); it != close_list.end();
           it++)
        stkvec[VS++] = *it;
      ...
      for (int i = size - 2; i < VS; i++) {
        ...
        for (int j = i - (size - 2); j < i; j++) stks_list.push_back(stkvec[j]);
        ...
        for (list<int>::iterator it = stks_list.begin(); it != stks_list.end();
             it++)
          printf(" %d", *it);
        printf(" }\n");
...
// Combine hypotheses A and B to create new hypothesis S using production 'S ->
// A B'
CellCYK *meParser::fusion(Sample *M, ProductionB *pd, Hypothesis *A,
                          Hypothesis *B, int N, double prob) {
  CellCYK *S = NULL;

  if (!A->parent->compatible(B->parent) || pd->prior == -FLT_MAX) return S;
  ...
  if (clusterF > 0.0) {
    ...
    if (grpen >= M->INF_DIST) return NULL;
  ...
  // Special treatment for binary productions that compose terminal symbols
  // (e.g. Equal --V--> Hline Hline)
  if (clase >= 0) {
    for (list<ProductionT *>::iterator it = G->prodTerms.begin();
         it != G->prodTerms.end(); it++) {
      ProductionT *prod = *it;
...
void meParser::parse_me(Sample *M) {
  ...
  // Init the parsing table with several multi-stroke symbol segmentation
  // hypotheses
  ...
  for (int talla = 2; talla <= N; talla++) {
    for (int a = 1; a < talla; a++) {
      ...
      for (CellCYK *c1 = tcyk.get(a); c1; c1 = c1->sig) {
        ...
        // Get the subset of regions close to c1 according to different spatial
        // relations
        ...
        for (list<CellCYK *>::iterator c2 = c1setH.begin(); c2 != c1setH.end();
             c2++) {
          for (list<ProductionB *>::iterator it = G->prodsH.begin();
               it != G->prodsH.end(); it++) {
            ...
            if (c1->noterm[pa] && (*c2)->noterm[pb]) {
              ...
              CellCYK *cd = fusion(M, *it, c1->noterm[pa], (*c2)->noterm[pb],
                                   M->nStrokes(), cdpr);

              if (!cd) continue;

              if (cd->noterm[ps]) {
                tcyk.add(talla, cd, ps,
                         G->esInit);  // Add to parsing table (size=talla)
              } else {
          ...
          for (list<ProductionB *>::iterator it = G->prodsSup.begin();
               it != G->prodsSup.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
          ...
          for (list<ProductionB *>::iterator it = G->prodsSub.begin();
               it != G->prodsSub.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
        ...
        for (list<CellCYK *>::iterator c2 = c1setV.begin(); c2 != c1setV.end();
             c2++) {
          for (list<ProductionB *>::iterator it = G->prodsV.begin();
               it != G->prodsV.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
          ...
          // prodsVe
          for (list<ProductionB *>::iterator it = G->prodsVe.begin();
               it != G->prodsVe.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
            ...
            if (c1->noterm[pa] && (*c2)->noterm[pb]) {
              double cdpr =
                  SPR.getVerProb(c1->noterm[pa], (*c2)->noterm[pb], true);
              if (cdpr <= 0.0) continue;
        ...
        for (list<CellCYK *>::iterator c2 = c1setU.begin(); c2 != c1setU.end();
             c2++) {
          for (list<ProductionB *>::iterator it = G->prodsV.begin();
               it != G->prodsV.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
            ...
            if (c1->noterm[pb] && (*c2)->noterm[pa]) {
              ...
              CellCYK *cd = fusion(M, *it, (*c2)->noterm[pa], c1->noterm[pb],
                                   M->nStrokes(), cdpr);
          ...
          for (list<ProductionB *>::iterator it = G->prodsVe.begin();
            ...
            if (c1->noterm[pb] && (*c2)->noterm[pa]) {
              double cdpr =
                  SPR.getVerProb((*c2)->noterm[pa], c1->noterm[pb], true);
              if (cdpr <= 0.0) continue;
        ...
        for (list<CellCYK *>::iterator c2 = c1setI.begin(); c2 != c1setI.end();
             c2++) {
          for (list<ProductionB *>::iterator it = G->prodsIns.begin();
               it != G->prodsIns.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
        ...
        // Mroot
        for (list<CellCYK *>::iterator c2 = c1setM.begin(); c2 != c1setM.end();
             c2++) {
          for (list<ProductionB *>::iterator it = G->prodsMrt.begin();
               it != G->prodsMrt.end(); it++) {
            if ((*it)->prior == -FLT_MAX) continue;
        ...
        for (int pps = 0; pps < c1->nnt; pps++) {
          ...
          if (c1->noterm[pps] && c1->noterm[pps]->prod &&
              c1->noterm[pps]->prod->tipo() == 'B') {
            logspace[b + c1->noterm[pps]->hi->parent->talla]->getS(
                c1, &c1setS);  // sup/sub-scripts union

            for (list<CellCYK *>::iterator c2 = c1setS.begin();
                 c2 != c1setS.end(); c2++) {
              if ((*c2)->x == c1->x && c1 != *c2) {
                for (list<ProductionB *>::iterator it = G->prodsSSE.begin();
                     it != G->prodsSSE.end(); it++) {
                  if ((*it)->prior == -FLT_MAX) continue;
                  ...
                  if (c1->noterm[pa] && (*c2)->noterm[pb] &&
                      ...
                      c1->noterm[pa]->hd->parent->compatible(
                          (*c2)->noterm[pb]->hd->parent)) {
                    // Subscript and superscript should start almost vertically
                    // aligned
                    if (abs(c1->noterm[pa]->hd->parent->x -
                            (*c2)->noterm[pb]->hd->parent->x) > 3 * M->RX)
                      continue;
                    // Subscript and superscript should not overlap
                    if (max((*it)->solape(c1->noterm[pa]->hd,
                                          (*c2)->noterm[pb]->hd),
                            (*it)->solape((*c2)->noterm[pb]->hd,
                                          c1->noterm[pa]->hd)) > 0.1)
                      continue;

                    float prob = c1->noterm[pa]->pr + (*c2)->noterm[pb]->pr -
                                 c1->noterm[pa]->hi->pr;

                    CellCYK *cd =
                        new CellCYK(G->noTerminales.size(), M->nStrokes());
                    ...
                    // Save the production of the superscript in order to
                    // recover it when printing the used productions
    ...
    if (talla < N) {
      ...
      logspace[talla] =
          new LogSpace(tcyk.get(talla), tcyk.size(talla), M->RX, M->RY);
    }
  ...
  // Free memory
  for (int i = 1; i < N; i++) delete logspace[i];
  delete[] logspace;
  ...
  if (M->getOutDot()) save_dot(mlh, M->getOutDot());
}
...
int meParser::tree2dot(FILE *fd, Hypothesis *H, int id) {
  ...
  if (!H->pt) {
    ...
    fprintf(fd, "%s%d -> %s%d [label=%c]\n", G->key2str(H->ntid), id,
            G->key2str(a), id + 1, H->prod->tipo());

    nid = tree2dot(fd, H->hi, id + 1);

    fprintf(fd, "%s%d -> %s%d [label=%c]\n", G->key2str(H->ntid), id,
            G->key2str(b), nid, H->prod->tipo());
