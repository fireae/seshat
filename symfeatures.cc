...
SymFeatures::SymFeatures(char *mav_on, char *mav_off) {
  ...
  // Read values online
  for (int i = 0; i < ON_FEAT; i++) fscanf(fd, "%lf", &means_on[i]);
  for (int i = 0; i < ON_FEAT; i++) fscanf(fd, "%lf", &stds_on[i]);
  ...
  // Read values offline
  for (int i = 0; i < OFF_FEAT; i++) fscanf(fd, "%lf", &means_off[i]);
  for (int i = 0; i < OFF_FEAT; i++) fscanf(fd, "%lf", &stds_off[i]);
...
DataSequence *SymFeatures::getOfflineFKI(int **img, int H, int W) {
  ...
  for (int x = 0; x < W; x++) {
    ...
    for (int i = 0; i < OFF_FEAT; i++) c[i] = 0;
    c[4] = H + 1;
